...
Public Sub Demo_Extension_Miniplot_BarChart()
    ...
    Dim db As New cDuck, csvPath As String, duckPath As String, outHtml As String, sql As String, a As Variant, html As String, csvContent As String
    ...
    ' --- (1) Create demo CSV if missing (self-contained demo) ---
    If Dir$(csvPath, vbNormal) = "" Then
        csvContent = _
            "product,revenue" & vbCrLf & _
            "iPhone,450" & vbCrLf & _
            "MacBook,380" & vbCrLf & _
            "iPad,290" & vbCrLf & _
            "AirPods,185" & vbCrLf & _
            "Watch,160" & vbCrLf

        Call WriteTextUtf8(csvPath, csvContent)
    End If
