...
Public Sub Demo_Json_Display() 'Import Json fichier + Display (création via DuckDB si absent)
    ...
    ' Chemin Windows + chemin DuckDB (avec /)
    pWin = ThisWorkbook.Path & "\instruments.json"
    p = Replace(pWin, "\", "/")
    ...
    ' --- Ensure JSON file exists (generated by DuckDB) ---
    If Dir$(pWin, vbNormal) = "" Then

        ' Petite table de démo en mémoire
        db.Exec "CREATE OR REPLACE TABLE demo_json(" & _
                "ISIN TEXT, NumeroContrat TEXT, Prix DOUBLE, ModifiedAt TIMESTAMP);"

        db.Exec "INSERT INTO demo_json VALUES " & _
                "('FR0000123456','C-001',101.25,TIMESTAMP '2025-01-01 10:00:00')," & _
                "('FR0000987654','C-002', 99.80,TIMESTAMP '2025-01-01 11:00:00');"

        ' Export JSON (DuckDB écrit le fichier)
        db.Exec "COPY (SELECT * FROM demo_json) TO " & SqlQ(p) & " (FORMAT JSON);"
    ...
    db.CloseDuckDb
    MsgBox "Import JSON OK : aperçu affiché", vbInformation
    Exit Sub
