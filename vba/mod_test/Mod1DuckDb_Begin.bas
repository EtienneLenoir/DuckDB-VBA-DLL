...
Public Sub Demo_OpenRO() 'Ouverture en lecture
    ...
    Dim db As New cDuck, ws As Worksheet, a As Variant, dbPath As String

    dbPath = ThisWorkbook.Path & "\DbDuckDb.duckdb"

    db.Init ThisWorkbook.Path
    db.ErrorMode = 2  '2=LogOnly (debug via duckdb_errors.log), 1=MsgBox, 0=Raise

    ' --- Ensure demo data exists (requires write access) ---
    ...
    If db.Scalar("SELECT COUNT(*) FROM Instruments;") = 0 Then
        db.Exec "INSERT INTO Instruments VALUES " & _
                "('FR0000123456','C-001',101.25,NOW())," & _
                "('FR0000987654','C-002', 99.80,NOW());"
    End If

    db.CloseDuckDb
    ' --- End ensure demo data ---
...
Public Sub Demo_OpenMemory() 'ouverture en RAM (:memory)
...
Public Sub Demo_Brackets() 'Demo Bracker
    ...
    db.Init ThisWorkbook.Path
    db.ErrorMode = 2  '2=LogOnly (debug via duckdb_errors.log), 1=MsgBox, 0=Raise
    db.OpenDuckDb ThisWorkbook.Path & "\DbDuckDb.duckdb"   ' ou ":memory:" pour un test RAM
